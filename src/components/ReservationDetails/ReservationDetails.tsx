...
const ReservationDetails: React.FC<TypeReservationDetailsProps> = (
  ...
  const renderDetailsItem = (key: TypeDetail) => {
    if (!cart.checkin || !cart.checkout || !cart.adults || !cart.children)
      return null;

    const items: any = {
      checkin: (
        <ReservationDetailsItem title="Check-in date" value={cart.checkin} />
      ),
      checkout: (
        <ReservationDetailsItem title="Check-out date" value={cart.checkout} />
      ),
      adults: <ReservationDetailsItem title="Adults" value={cart.adults} />,
      children: (
        <ReservationDetailsItem title="Children" value={cart.children} />
      ),
      room:
  ...
  return (
    <div className={elClasses.join(" ")}>
      <h2 className={styleClasses["reservation-details__title"]}>
        ...
        {selectedHotel?.details?.city && (
          <span>{`(${selectedHotel.details.city})`}</span>
        )}
      </h2>
